...
with open(submission_metadata, 'r') as stream:
    try:
...
for item in submissions.keys():
    ...
    newname = os.path.join(d2,'Color-orig.pdf')
    for dd in [ d1, d2 ]:
        try:
            os.mkdir(dd)
        except FileExistsError as mkdirex:
...
# Below is a modified version of repl-font.py
# from https://github.com/pymupdf/PyMuPDF-Utilities/tree/master/font-replacement

# Main changes:
#   1. Use the fontReplacements dictionary from above instead of a JSON file
#      to control the replacements.
#   2. Apply the Unicode bidi algorithm to fix the text direction, as the
#      files being processed would otherwise have the Hebrew appear backwards
#      after the font replacement.
#   3. Add the clear_data method.
#   4. Add the build_repl_table2 method - based roughly on the regular
#      build_repl_table method.
#   5. The main code loops over and processes all files in the pdffiles array.
#   6. The method used to generate the final output file name is different,
#      customized for the namimg convention needed by the Technion.
#
"""
...
def build_repl_table2(doc):
    ...
    for oldfont in fontReplacements.keys():
        newfont = fontReplacements.get(oldfont).strip()
...
# ------------------
# MODIFIED main to loop over files
# ------------------

# List files
print('Found')
for infilename in pdffiles:
    clear_data()
    indoc = fitz.open(infilename)  # input PDF

    build_repl_table2(indoc)

    if new_fontnames == {}:
        print('No fonts to replace in {f}'.format(f=infilename))
        continue
